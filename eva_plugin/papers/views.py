...
logger = logging.getLogger('eva_plugin.papers')


class PaperLoadHandler(BaseHandler):  # метод отвечающяя за загрузку файл в папку

    def post(self, request):  # метод который положит файл в папку
        files = request.FILES
        result = {'status': 'success'}

        reports_path = os.path.join(STATIC_CONF['static_path'], 'reports')  # путь куда будем сохранять файл

        try:
            _file: UploadedFile = files['file']
            saving_full_path = os.path.join(reports_path, _file.name)  # тут к пути еще добовляем имя файла
            with open(saving_full_path, 'wb') as f:  # то открывает его (и создает видимо)
                try:
                    f.write(_file.read())  # и записывает в него данные с фронта
                except Exception:
                    result['status'] = 'failed'
                    result['description'] = 'fail is not write'
        except AttributeError:
            result['status'] = 'failed'
            result['description'] = 'fail is not send'
        ...
        return Response(result)  # передаем успешное выполнение запроса


class PapersHandler(BaseHandler):  # метод возвращающий все файлы в папке на фронт

    def get(self, request):  # метод который вернет все файлы в папке
        reports_path = STATIC_CONF['static_path'] + 'reports'  # путь до нужной папки
...
class PaperHandler(BaseHandler):  # метод который изменит файл с фротна и вернет ссылку на новый
    def __init__(self, *args, **kwargs):
        super(PaperHandler, self).__init__(*args, **kwargs)
        self.static_dir_name = 'storage'


    def post(self, request):  # метод который изменит файл на основе данных с фронта и отдаст ссылку на новый
        file_name = request.data['file']
        reports_path = os.path.join(STATIC_CONF['static_path'], 'reports')  # путь до папки откуда брать файлы
        try:  # попытаемся получить готовые данные с фронта
            ...
            return Response({'description': 'data is not available',
                        'status': 'failed'})  # выведем сообщение что данные поулчить не удалось

        if data['status'] == 'failed':
            return Response({'description': 'cache is cleared and search is gone', 'status': 'failed'})
        else:
            data = data['data']
            ...
            return Response(result)  # вернем результат выполнения метода
    ...
    def work_docx(self, path, data, name_file):  # метод для работы с xlsx файлами
        ...
        files = []
        html = []
        reports_path = os.path.join(STATIC_CONF['static_path'],
    ...
    def get_data(self, cid):  # метод для поулчения данных запроса
        body = []
        try:  # попробуем
            path_to_cache_dir = os.path.join(MEM_CONF['path'],
