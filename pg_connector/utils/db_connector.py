...
class PostgresConnector:
    ...
    def check_permission_exists(self, permission_name):
        permission_id = self.pg.execute_query("SELECT id from permission WHERE name = %s;",
    ...
    def get_permissions_data(self, *, user_id=None, names_only=False):
        if user_id:
            ...
            for role in user_roles:
                role_id = self.pg.execute_query("SELECT id FROM role WHERE name = %s;", params=(role,))
                role_permissions = self.pg.execute_query("SELECT * FROM permission WHERE id IN "
                                                      "(SELECT permission_id FROM role_permission WHERE role_id = %s);",
                                                         params=(role_id,), fetchall=True, as_obj=True)
                permissions.extend(role_permissions)
            permissions = list({v['id']: v for v in permissions}.values())
        else:
            permissions = self.pg.execute_query("SELECT * FROM permission;", fetchall=True, as_obj=True)
        if names_only:
            permissions = [p['name'] for p in permissions]
        else:
            for permission in permissions:
                roles = self.pg.execute_query("SELECT name FROM role WHERE id IN "
    ...
    def get_permission_data(self, permission_id):
        permission = self.pg.execute_query("SELECT * FROM permission WHERE id = %s;",
    ...
    def add_permission(self, *, name, roles):
        ...
        with self.pg.transaction('add_permission_data') as conn:
            permission_id = self.pg.execute_query("INSERT INTO permission (name) VALUES (%s) RETURNING id;",
                                                  conn=conn, params=(name,))
            if roles:
                for role in roles:
                    self.pg.execute_query("INSERT INTO role_permission (role_id, permission_id) "
                                       "VALUES ((SELECT id FROM role WHERE name = %s), %s);",
    ...
    def update_permission(self, *, permission_id, name, roles=None):
        if name:
            self.pg.execute_query("UPDATE permission SET name = %s WHERE id = %s;",
                ...
                for role in roles_for_add:
                    self.pg.execute_query("INSERT INTO role_permission (role_id, permission_id) "
                                       "VALUES ((SELECT id FROM role WHERE name = %s), %s);",
                                          conn=conn, params=(role, permission_id,), with_fetch=False)
                if roles_for_delete:
                    self.pg.execute_query("DELETE FROM role_permission WHERE role_id IN "
    ...
    def delete_permission(self, permission_id):
        self.pg.execute_query("DELETE FROM permission WHERE id = %s;", params=(permission_id,),
                              with_commit=True, with_fetch=False)
        return permission_id
